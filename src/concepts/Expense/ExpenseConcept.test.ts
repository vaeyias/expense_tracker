...
Deno.test("ExpenseConcept - Operational Principle", async (t) => {
  ...
  await t.step(
    "Create an expense, then edit it, then retrieve it",
    async () => {
      const creator: ID = "user:1" as ID;
      const payer: ID = "user:2" as ID;
      const group: ID = "group:1" as ID;
      ...
      assertEquals(
        typeof expenseId,
        "string",
        "createExpense should return an expense ID",
      );

      // Retrieve the created expense to verify
      const createdExpense = await expenseConcept._getExpenseById({
        expenseId,
      });
      assertEquals(
        createdExpense?.title,
        initialExpenseDetails.title,
        "Retrieved expense should match created details",
      );
      ...
      assertEquals(
        typeof editResult.newExpense,
        "string",
        "editExpense should return the edited expense ID",
      );

      // Retrieve the edited expense to verify
      const editedExpense = await expenseConcept._getExpenseById({ expenseId });
      assertEquals(
        editedExpense?.totalCost,
        editedDetails.totalCost,
        "Edited expense should have updated totalCost",
      );
      assertEquals(
        editedExpense?.description,
        editedDetails.description,
        "Edited expense should have updated description",
      );
      assertEquals(
        editedExpense?.debtMapping,
        editedDetails.debtMapping,
        "Edited expense should have updated debtMapping",
      );
    },
  );

  await t.step(
    "Create an expense, then delete it, then try to retrieve it",
    async () => {
      const creator: ID = "user:3" as ID;
      const payer: ID = "user:4" as ID;
      const group: ID = "group:2" as ID;
      ...
      assertEquals(
        ...
        expenseId,
        "deleteExpense should return the deleted expense ID",
      );
...
Deno.test("ExpenseConcept - Interesting Scenarios", async (t) => {
  ...
  const user1: ID = "user:1" as ID;
  const user2: ID = "user:2" as ID;
  const user3: ID = "user:3" as ID;
  const group1: ID = "group:1" as ID;
  const group2: ID = "group:2" as ID;
  ...
  await t.step("Create an expense with minimal required fields", async () => {
    ...
    assertEquals(
      expense?.description,
      undefined,
      "Expense description should be undefined",
    );
  });
  ...
  await t.step("Edit an expense with only one field", async () => {
    ...
    const editedExpense = await expenseConcept._getExpenseById({
      expenseId: editResult.newExpense,
    });
    assertEquals(
  ...
  await t.step(
    "Attempt to create an expense with totalCost <= 0 (should fail)",
    async () => {
      await assertRejects(
        async () => {
  ...
  await t.step("Attempt to edit an expense that does not exist", async () => {
    ...
    assertEquals(
      editResult.newExpense,
      "",
      "Editing a non-existent expense should return an empty string",
    );
  });
  ...
  await t.step("Attempt to delete an expense that does not exist", async () => {
    const nonExistentExpenseId: Expense = "expense:non-existent";
    const deleteResult = await expenseConcept.deleteExpense({
      expenseToDelete: nonExistentExpenseId,
    });
    assertEquals(
      deleteResult.deletedExpense,
      "",
      "Deleting a non-existent expense should return an empty string",
    );
  });
  ...
  await t.step("Retrieve all expenses for a group", async () => {
    ...
    const expensesInGroup1 = await expenseConcept._getExpensesByGroup({
      group: group1,
    });
    assertEquals(
      expensesInGroup1.length,
      2,
      "Should retrieve exactly two expenses for group1",
    );
    expensesInGroup1.forEach((exp) =>
      assertEquals(
        exp.group,
        group1,
        "All retrieved expenses should belong to group1",
      )
    );

    const expensesInGroup2 = await expenseConcept._getExpensesByGroup({
      group: group2,
    });
    assertEquals(
      expensesInGroup2.length,
      1,
      "Should retrieve exactly one expense for group2",
    );
    assertEquals(
      expensesInGroup2[0].group,
      group2,
      "The retrieved expense should belong to group2",
    );
  });
